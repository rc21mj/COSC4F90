...
std::vector<double> LteRealisticChannelModel::getSINR(LteAirFrame *frame, UserControlInfo* lteInfo)
{
   ...
   std::ofstream speedFile("/home/ritika/Downloads/Ritika_Project/Project_GCN_LSTM_HO/simu5G/src/Datafiles/speed.txt");
   speedFile << globalSpeed << endl;
   speedFile.close();
   ...
   //=================== END PARAMETERS SETUP =======================
    ...
    double sqrDistance = phy_->getCoord().distance(coord);
    MacNodeId nodeId_chnlMdl = ueId;
    MacNodeId towerId_chnlMdl = eNbId;

    if(nodeId_chnlMdl>=2049)
        nodeId_chnlMdl = 10000+nodeId_chnlMdl;
    else if(nodeId_chnlMdl>=1025 && nodeId_chnlMdl<2049)
        nodeId_chnlMdl = 10000+nodeId_chnlMdl+1024;
//    std::cout << "nodeId_chnlMdl: " << nodeId_chnlMdl << " towerId_chnlMdl: " << towerId_chnlMdl << " sqrDistance: " << sqrDistance << endl;

    std::ofstream distFile("/home/ritika/Downloads/Ritika_Project/Project_GCN_LSTM_HO/simu5G/src/Datafiles/dist.txt");
    distFile << sqrDistance << endl;
    distFile.close();
