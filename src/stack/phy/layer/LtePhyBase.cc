...
void LtePhyBase::handleMessage(cMessage* msg) //this method is called
{
    std::string msgname = msg->getName();

    if(msgname.compare("NazaninVoip") == 0){
        std::cout <<"Nazanin VOIP received!" << std::endl;
    }
//    std::cout << "(LtePhyBase::handleMessage) -> msg's name" << msg->getName()  << endl;
//    std::cout << "(LtePhyBase::handleMessage) -> " << msg->getSenderModule() << " from channel" << endl;
//    std::cout << "LtePhyBase::handleMessage - new message received" << endl;
    //std::cout << "LtePhyBase::handleMessage - new message received" << endl;

//    double speed_NR = LteRealisticChannelModel_->computeSpeed(nodeId_, radioPos)
//    std::cout << "NRPhyUe node id " << nodeId_  << " (computeSpeed) speedNR " << speed_NR << endl;

    if (msg->isSelfMessage())
    {
//        std::cout<< msg->getName() << " is self message"<< std::endl;
        handleSelfMessage(msg);
    }
    // AirFrame
    else if (msg->getArrivalGate()->getId() == radioInGate_)
    {
//        std::cout<< msg->getName() << " is airframe message"<< std::endl;
        handleAirFrame(msg);
    }
    ...
    {
//        std::cout<< msg->getName() << " is stack message"<< std::endl;
        handleUpperMessage(msg);
    }
    // unknown message
    else
    {
//        std::cout<< msg->getName() << " is unknown"<< std::endl;
...
//    std::cout << "LtePhyBase::handleControlMsg" << endl;
    return;
}

LteAirFrame *LtePhyBase::createHandoverMessage() //this method called
{
    // broadcast airframe
...
// sending messages from towers/Servers to vehicles
void LtePhyBase::handleUpperMessage(cMessage* msg) //it is called
{


    EV << "LtePhyBase: message from stack" << endl;

    LteAirFrame* frameTemp = static_cast<LteAirFrame*>(msg);
...
//        std::cout << "frame received" << endl;
    }
    else
    {
        // create LteAirFrame and encapsulate the received packet
//        std::cout << "frame created" << endl;
    ...
    EV << "LtePhyBase: " << nodeTypeToA(nodeType_) << " with id " << nodeId_
       << " sending message to the air channel. Dest=" << lteInfo->getDestId() << endl;
//    std::cout << "LtePhyBase: " << nodeTypeToA(nodeType_) << " with id " << nodeId_
//            << " sending message to the air channel. Dest=" << lteInfo->getDestId() << endl;
    sendUnicast(frame);
...
{
    //std::cout << "LtePhyBase::initializeChannelModel " << nodeId_ << endl;
...
void LtePhyBase::sendBroadcast(LteAirFrame *airFrame) //this method is calling
{
