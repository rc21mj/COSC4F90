...
void LtePhyEnbD2D::requestFeedback(UserControlInfo* lteinfo, LteAirFrame* frame, Packet* pktAux)
{
    ...
    //std::cout << NOW << " LtePhyEnbD2D::requestFeedback " << endl;
...
void LtePhyEnbD2D::handleAirFrame(cMessage* msg)
{
    ...
    //std::cout << "LtePhyEnbD2D::handleAirFrame - received new LteAirFrame with ID " << frame->getId() << " from channel" << endl;
    ...
    if (lteInfo->getDestId() != nodeId_)
    {
        ...
        std::cout << "Frame MacNodeId: " << lteInfo->getDestId() << endl;
    ...
    //std::cout << "connectedNodeId_  " << connectedNodeId_  << endl;
  ...
  //  std::cout << "Handled LteAirframe with ID " << frame->getId() << " with result "<< (result ? "RECEIVED" : "NOT RECEIVED") << endl;

    auto pkt = check_and_cast<inet::Packet *>(frame->decapsulate());
   // std::cout << "SHajib::LtePhyBase::handleUpperMessage: 2 SHAJIBSourceID  " << lteInfo->getSourceId() << " Dest id: " << lteInfo->getDestId() << " pkt1->getName(): " << pkt->getName() << std::endl;
