...
//        std::cout << "hiiiiiiiiiiiiiiiiiiiiiiii" << endl;
...
bool LtePhyEnb::handleControlPkt(UserControlInfo* lteinfo, LteAirFrame* frame)
{
    ...
    //std::cout << "Received control pkt " << endl;
...
void LtePhyEnb::handleAirFrame(cMessage* msg) //not called
{

    std::cout << "(LtePhyEnb::handleAirFrame) -> voip msg's name" << msg->getName() << " from channel" << endl;
...
//    std::cout << "LtePhy: received new LteAirFrame with ID " << frame->getId() << " from channel" << endl;
...
//    std::cout << "Handled LteAirframe with ID " << frame->getId() << " with result "
//            << (result ? "RECEIVED" : "NOT RECEIVED") << endl;
...
void LtePhyEnb::requestFeedback(UserControlInfo* lteinfo, LteAirFrame* frame, Packet* pktAux)
{
    ...
    //std::cout << NOW << " LtePhyEnb::requestFeedback " << endl; //not called
...
//    if(this->nodeId_ == nodeId_)
//        this->count++;
//        std::cout << "<node id> " << nodeId_ << " dest id " << lteinfo->getDestId() <<
//                " <source id> " << lteinfo->getSourceId() << " <count> " << count << endl;
        ...
        if( debug )
        {
            ...
            for (it = vec.begin(); it != vec.end(); ++it)
            {
                for (jt = it->begin(); jt != it->end(); ++jt)
                {
                    ...
                    std::cout << endl << "Node:" << id << endl;
                    TxMode t = jt->getTxMode();
                    EV << "TXMODE: " << txModeToA(t) << endl;
                    std::cout << "TXMODE: " << txModeToA(t) << endl;
                    if (jt->hasBandCqi())
                    {
