...
double minLoad = -5;
int numHO = 0, failHO = 0, pingpongHO = 0;
double timeHO_LtePhyUe = 0, eachTimeHO_LtePhyUe = 0;
double plrHO_LtePhyUe = 0, eachPlrHO_LtePhyUe = 0;
double predictedTowerLoad = 0.0, predictedRSSI = 0.0, predictedDistance = 0.0;
bool isPerformedAnalysisLtePhyUe = false, isSavePerformanceAnalysis = false, isRunPythonScript = false;

struct Metrics {
    int numHO_speed = 0;
    int failHO_speed = 0;
    int pingpongHO_speed = 0;
    double timeHO_speed = 0;
    double plrHO_speed = 0;
};
std::map<std::string, Metrics> speedMetrics;
std::string category;
...
double LtePhyUe::calculateEachTowerLoad(UserControlInfo* lteInfo, LteAirFrame* frame)
{
    int index = lteInfo->getSourceId()-1;

    int totalVehicles = (int)getDoubleValueFile(filePath_LtePhyUe+"nodeCount.txt");
//    std::cout << "totalVehicles " << totalVehicles << endl;

    if (bsLoad[index]<totalVehicles)
        bsLoad[index]++;
//    std::cout << "bsLoad[index] " << bsLoad[index] << endl;

    bsLoadCal1Weighted = (bsLoad[index] + 5) / (std::accumulate(bsLoad, bsLoad + NUM_TOWERS, 0) + 5);
//    std::cout << "bsLoadCal1Weighted " << bsLoadCal1Weighted << endl;
...
double LtePhyUe::getDoubleValueFile(std::string filepath)
{
...
void LtePhyUe::writeToCSV(const std::string& filename, double simtimeDbl, MacNodeId vehicleId, MacNodeId towerId, double rssi, double distance, double towerLoad)
{
    ...
    if (file.tellp() == 0)
    {
        file << "Time,VehicleID,TowerID,RSSI,Distance,TowerLoad\n";
    }
...
//        std::cout << simtimeDbl << " " << vehicleId << " " << towerId << " " << rssi << " " << distance << " " << towerLoad << std::endl;

        // Write to file in CSV format
        file << simtimeDbl << "," << vehicleId << "," << towerId << "," << rssi << "," << distance << "," << towerLoad << "\n";
...
// Function to clear half the data in the file
void LtePhyUe::clearHalfFileData(const std::string& filePath)
{
...
//        std::cerr << "Error: Could not open file " << filePath << std::endl;
    ...
    // Determine the number of lines to keep (half the file)
    size_t halfLines = lines.size() / 2;

    // Write back only the first half of the lines
...
void LtePhyUe::callingPython(const std::string& filename)
{
    // Ensure the filename includes the Python interpreter
    std::string command = "python3 " + filename;

    // Open a process to execute the Python script
    FILE* pipe = popen(command.c_str(), "r");
    if (!pipe) {
//        std::cerr << "Error opening pipe for executing Python script: " << filename << std::endl;
        return;
    }

    // Buffers to capture specific outputs
//    double predictedTowerLoad = 0.0;
//    double predictedRSSI = 0.0;
//    double predictedDistance = 0.0;

    // Buffer to store lines read from the Python script output
    char buffer[256];
    while (fgets(buffer, sizeof(buffer), pipe) != NULL) {
        std::string line(buffer);

        // Parse the output for specific keywords and extract values
        if (line.find("Final Predicted Tower Load:") != std::string::npos) {
            sscanf(line.c_str(), "Final Predicted Tower Load: %lf", &predictedTowerLoad);
        } else if (line.find("Final Predicted RSSI:") != std::string::npos) {
            sscanf(line.c_str(), "Final Predicted RSSI: %lf", &predictedRSSI);
        } else if (line.find("Final Predicted Distance:") != std::string::npos) {
            sscanf(line.c_str(), "Final Predicted Distance: %lf", &predictedDistance);
        }

        // Optionally print the output to console for debugging
//        std::cout << "Python Output: " << line;
    ...
    // Print captured values to console
//    std::cout << "Captured Predictions:\n";
//    std::cout << "Tower Load: " << predictedTowerLoad << "\n";
//    std::cout << "RSSI: " << predictedRSSI << "\n";
//    std::cout << "Distance: " << predictedDistance << "\n";
...
void LtePhyUe::handoverHandler(LteAirFrame* frame, UserControlInfo* lteInfo)
{
    ...
    //calling tower load
    eachTowerLoad = calculateEachTowerLoad(lteInfo, frame);

    //speed
    speedVehicleM = getDoubleValueFile(filePath_LtePhyUe+"speed.txt");
    speedVehicleKM = speedVehicleM  * 3.6; // converting m/h to km/h
//    std::cout << "speedVehicleKM: " << speedVehicleKM << endl;
    double speedKM = speedVehicleKM;

    // Determine the speed category
    if (speedKM >= 0 && speedKM < 20) category = "SPEED_020";
    else if (speedKM >= 20 && speedKM < 40) category = "SPEED_2140";
    else if (speedKM >= 40 && speedKM < 60) category = "SPEED_4160";
    else if (speedKM >= 60 && speedKM < 80) category = "SPEED_6180";
    else if (speedKM >= 80 && speedKM < 100) category = "SPEED_81100";
    else if (speedKM >= 100 && speedKM < 120) category = "SPEED_101120";
    else if (speedKM >= 120 && speedKM < 140) category = "SPEED_121140";
    else if (speedKM >= 140 && speedKM < 160) category = "SPEED_141160";
    ...
    eachPlrHO_LtePhyUe = getDoubleValueFile(filePath_LtePhyUe+"eachPlrHO.txt");
    speedMetrics[category].plrHO_speed = eachPlrHO_LtePhyUe + speedMetrics[category].plrHO_speed;

    //time
    timeHO_LtePhyUe = getDoubleValueFile(filePath_LtePhyUe+"timeHO.txt");
    eachTimeHO_LtePhyUe = getDoubleValueFile(filePath_LtePhyUe+"eachTimeHO.txt");
    speedMetrics[category].timeHO_speed = eachTimeHO_LtePhyUe + speedMetrics[category].timeHO_speed;

//    std::cout << "Simtime: " << simTime() << " Node: " << nodeId_ << " Tower: " << lteInfo->getSourceId() << " RSSI: " << rssi << " Min RSSI: " << minRssi_ << endl;
...
//            << " Time HO: " << timeHO_LtePhyUe << " PLR HO: " << plrHO_LtePhyUe << endl;
    ...
    if ((int)simTime().dbl() % 11 == 0)
    {
//        clearFileData(filePath_LtePhyUe+"dataStorage.csv");
//        clearHalfFileData(filePath_LtePhyUe+"dataStorage.csv");
        isRunPythonScript = false;
    }
...
//    if (rssi > candidateMasterRssi_ + hysteresisTh_)
    {
        ...
        else
        {
            ...
            numHO++;
            speedMetrics[category].numHO_speed++;
            candidateMasterId_ = lteInfo->getSourceId();
            if(stored_masterId == candidateMasterId_)
            {
                pingpongHO++;
                speedMetrics[category].pingpongHO_speed++;
            }
            stored_masterId = candidateMasterId_;
            candidateMasterRssi_ = rssi;
    ...
    else
    {
        if (lteInfo->getSourceId() == masterId_)
        {
            ...
            else  // lost connection from current master
            {
                ...
                {
                    failHO++;
                    speedMetrics[category].failHO_speed++;
...
//    printMetrics();
    performanceAnalysis_LtePhyUe();
    savePerformanceAnalysis();

    delete frame;
}

void LtePhyUe::printMetrics() {
    std::cout << "Speed - ";
    for (const auto &entry : speedMetrics) {
        const auto &category = entry.first;
        const auto &metrics = entry.second;
        std::cout << category << ": "
                  << "HO=" << metrics.numHO_speed
                  << ", Fail=" << metrics.failHO_speed
                  << ", Pingpong=" << metrics.pingpongHO_speed
                  << ", Time=" << metrics.timeHO_speed
...
void LtePhyUe::savePerformanceAnalysis()
{
    if (simTime().dbl() >= 100 && !isSavePerformanceAnalysis)
    {
        int result;
        time_t t = time(0);
        ...
        char oldname[] = "/home/ritika/Downloads/Ritika_Project/omnetpp-6.0/Results_CMD/outputCMD.txt";
        std::string str(buffer);
        std::string strNewFile = "/home/ritika/Downloads/Ritika_Project/omnetpp-6.0/Results_CMD/" + str + ".txt";
        int len = strNewFile.length();
        char buffer_new[len + 1];
        strcpy(buffer_new, strNewFile.c_str());
        result = rename(oldname, buffer_new);

        isSavePerformanceAnalysis = true;
    }
}
