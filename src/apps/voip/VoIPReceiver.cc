...
double throughput = 0, plr =0, plr_count = 0;
...
double VoIPReceiver::eModel(simtime_t delay, double loss)
{
    ...
    plr = plr + p;
    plr_count++;
    double avg_plr = plr / plr_count;
//    std::cout << "VoIPReceiver: PLR: " << avg_plr << endl;

    std::ofstream plrHOFile("/home/ritika/Downloads/Ritika_Project/Project_GCN_LSTM_HO/simu5G/src/Datafiles/plrHO.txt");
    plrHOFile << avg_plr << endl;
    plrHOFile.close();

    std::ofstream eachPlrHOFile("/home/ritika/Downloads/Ritika_Project/Project_GCN_LSTM_HO/simu5G/src/Datafiles/eachPlrHO.txt");
    eachPlrHOFile << p << endl;
    eachPlrHOFile.close();
